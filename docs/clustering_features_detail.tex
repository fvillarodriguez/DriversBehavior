...
La lógica de cálculo descrita anteriormente está implementada principalmente en el módulo \texttt{src/clustering.py}. A continuación se detalla el flujo de ejecución de las funciones clave para fines de auditoría.

\subsection{Limpieza de Datos: \texttt{clean\_flujos\_for\_clustering}}
Esta función recibe el DataFrame de flujos brutos y realiza la preparación inicial:
\begin{enumerate}
    \item \textbf{Validación de Columnas:} Se asegura que existan las columnas esenciales (timestamp, velocidad, pórtico, carril, patente).
    \item \textbf{Limpieza de Patentes:} Se normalizan las patentes (alfanuméricos, mayúsculas) y se filtran aquellas con longitud inválida (fuera del rango $[5, 6]$).
    \item \textbf{Filtrado Básico:} Se eliminan filas con datos nulos en campos críticos y se conservan solo los carriles 1, 2 y 3.
    \item \textbf{Deduplicación:} Se eliminan duplicados exactos de la tupla (Patente, Pórtico, Carril, Timestamp). Si existen duplicados con diferentes velocidades, se prioriza determinísticamente (ordenamiento merge sort presenvando el primero).
    \item \textbf{Winsorization:} Se limitan los valores extremos de velocidad por grupo (Pórtico, Carril) usando los percentiles definidos (default $1\%$ y $99\%$).
\end{enumerate}

\subsection{Cálculo de Features: \texttt{Clusterization}}
Esta es la función principal que orquesta el cálculo de variables.

\subsubsection{1. Métricas Simples y Pre-cálculo}
Se calculan métricas que no requieren ordenamiento temporal complejo:
\begin{itemize}
    \item \textbf{Total de Pasadas (\texttt{total\_passes}):} Conteo simple de registros por patente.
    \item \textbf{Velocidad Promedio (\texttt{avg\_speed\_kmh}):} Promedio aritmético de la velocidad.
    \item \textbf{Uso de Carril:} Se construye una tabla pivote para contar pasadas por carril y dividir por el total.
\end{itemize}

\subsubsection{2. Velocidad Relativa}
\begin{itemize}
    \item Se calcula la velocidad promedio del flujo en ventanas de 5 minutos por pórtico (\texttt{interval\_speed\_mean}).
    \item Se cruza este promedio con cada evento para calcular \texttt{relative\_speed} $= v_i / \bar{V}_{5min}$.
\end{itemize}

\subsubsection{3. Métricas de Interacción (Headway, Conflicto)}
Esta etapa es crítica y sensitiva al orden de los datos:
\begin{enumerate}
    \item \textbf{Ordenamiento:} Los datos se ordenan estrictamente por Pórtico, Carril y Timestamp.
    \item \textbf{Operaciones Vectorizadas (Shift):} Se crean columnas desplazadas (\texttt{shift(1)}) para comparar el evento actual con el vehículo inmediatamente anterior en el mismo carril/pórtico.
    \item \textbf{Cálculo de Headway:} Diferencia de tiempo entre el evento actual y el anterior. Se descartan (NaN) los headways mayores a \texttt{max\_headway\_s} (60s).
    \item \textbf{Cálculo de Conflicto:}
        \begin{itemize}
            \item Se calcula $TTC = \frac{Headway \cdot v_{actual}}{v_{actual} - v_{anterior}}$.
            \item Solo es válido si $v_{actual} > v_{anterior}$.
            \item Se marca conflicto si $TTC < TTC_{max}$ (donde $TTC_{max}$ depende del pórtico).
        \end{itemize} 
    \item \textbf{Agregación:} Se suman y cuentan los eventos válidos por patente para obtener promedios.
\end{enumerate}

\subsubsection{4. Cambios de Pista (\texttt{Lane Changes})}
\begin{itemize}
    \item Se ordenan los datos por Patente y Timestamp.
    \item Se compara el carril actual con el anterior. Si son diferentes (y es la misma patente), se cuenta como cambio.
    \item La tasa se normaliza por ($N_P - 1$).
\end{itemize}

\subsubsection{5. Ponderación (Si aplica)}
Si se activa \texttt{monthly\_weighting}, las métricas se calculan primero agrupando por Patente-Mes, y luego se hace un promedio ponderado por la cantidad de viajes del mes para obtener el valor final de la patente.
